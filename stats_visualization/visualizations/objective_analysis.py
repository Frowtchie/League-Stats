...
def extract_objective_data(player_puuid: str, matches_dir: str = "matches") -> Dict[str, Any]:
    """
    ...
    objective_data = {
        ...
        'first_objectives': {'dragon': 0, 'baron': 0, 'herald': 0, 'tower': 0, 'blood': 0},
        'total_games': 0
    }
...
def plot_objective_control(player_name: str, objective_data: Dict[str, Any]):
    ...
    for i, (obj_name, data_key) in enumerate(zip(objectives, data_keys)):
        ...
        if not player_objs:
            ax.text(0.5, 0.5, f'No {obj_name.lower()} data', 
        ...
        wins_when_ahead = sum(1 for p, e, w in zip(player_objs, enemy_objs, objective_data[data_key]['wins'])
                             if p > e and w)
        games_ahead = sum(1 for p, e in zip(player_objs, enemy_objs) if p > e)
        
        if games_ahead > 0:
            win_rate_ahead = wins_when_ahead / games_ahead * 100
            ax.text(0.02, 0.98, f'Win rate when ahead: {win_rate_ahead:.1f}%',
...
def main():
    ...
    parser.add_argument("--chart", type=str, choices=["control", "first", "correlation", "all"],
    ...
    print(f"Analyzing objective data for {args.player}...")
    objective_data = extract_objective_data(player_puuid, args.matches_dir)
    ...
    if args.chart == "control" or args.chart == "all":
        print(f"Generating objective control chart for {args.player}...")
        plot_objective_control(args.player, objective_data)
    
    if args.chart == "first" or args.chart == "all":
        print(f"Generating first objectives chart for {args.player}...")
        plot_first_objectives(args.player, objective_data)
        
    if args.chart == "correlation" or args.chart == "all":
        print(f"Generating objective-win correlation chart for {args.player}...")
        plot_objective_win_correlation(args.player, objective_data)
