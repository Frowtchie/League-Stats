...
def extract_economy_data(player_puuid: str, matches_dir: str = "matches") -> Dict[str, Any]:
    """
...
def plot_farming_performance(player_name: str, economy_data: Dict[str, Any]):
    ...
    if len(cs_per_min) > 1:
        ...
        p = np.poly1d(z)
        ax3.plot(game_numbers, p(game_numbers), "r--", alpha=0.8, label=f'Trend: {z[0]:.2f} CS/min per game')
        ax3.legend()
    ...
    for i, (lower, upper) in enumerate([
        ...
        games_in_range = [(cs, win) for cs, win in zip(cs_per_min, wins) 
...
def plot_gold_efficiency(player_name: str, economy_data: Dict[str, Any]):
    ...
    if len(total_gold) > 1:
        ...
        p = np.poly1d(z)
        ax3.plot(sorted(total_gold), p(sorted(total_gold)), "r--", alpha=0.8,
    ...
    if len(damage_per_gold) > 1:
        ...
        p = np.poly1d(z)
        ax4.plot(game_numbers, p(game_numbers), "r--", alpha=0.8, 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate farming and economy analysis visualizations")
    ...
    parser.add_argument("--chart", type=str, choices=["farming", "gold", "roles", "all"],
    ...
    print(f"Analyzing economy data for {args.player}...")
    economy_data = extract_economy_data(player_puuid, args.matches_dir)
    
    if economy_data['total_games'] == 0:
        print(f"No matches found for {args.player}")
        return
    
    # Generate requested charts
    if args.chart == "farming" or args.chart == "all":
        print(f"Generating farming performance chart for {args.player}...")
        plot_farming_performance(args.player, economy_data)
    
    if args.chart == "gold" or args.chart == "all":
        print(f"Generating gold efficiency chart for {args.player}...")
        plot_gold_efficiency(args.player, economy_data)
        
    if args.chart == "roles" or args.chart == "all":
        print(f"Generating role economy comparison for {args.player}...")
        plot_role_economy_comparison(args.player, economy_data)
