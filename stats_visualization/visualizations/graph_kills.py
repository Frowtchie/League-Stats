...
def extract_kills_data(player_puuid: str, matches_dir: str = "matches") -> Dict[str, Any]:
    """
...
def plot_kills_analysis(player_name: str, kills_data: Dict[str, Any]):
    ...
    ax1.plot(game_numbers, kills_data['assists'], 's-', alpha=0.7, color='blue', label='Assists')
    ...
    if len(kills_data['kills']) > 1:
        ...
        ax1.plot(game_numbers, np.poly1d(kills_trend)(game_numbers), 
                '--', alpha=0.8, color='darkred', label=f'Kills trend: {kills_trend[0]:.2f}/game')
        ax1.plot(game_numbers, np.poly1d(assists_trend)(game_numbers), 
                '--', alpha=0.8, color='darkblue', label=f'Assists trend: {assists_trend[0]:.2f}/game')
    ...
    loss_kda = [kda for kda, w in zip(kills_data['kda_ratios'], kills_data['wins']) if not w]
...
def plot_detailed_performance(player_name: str, kills_data: Dict[str, Any]):
    ...
    champion_stats = defaultdict(lambda: {
        'kills': [], 'deaths': [], 'assists': [], 'kda': [], 'games': 0
    })
    ...
    top_champions = dict(sorted(filtered_champions.items(), 
                               key=lambda x: x[1]['games'], reverse=True)[:6])
...
def main():
    ...
    parser.add_argument("--detailed", action="store_true",
                       help="Show detailed champion breakdown")
    ...
    # Extract kills data
    print(f"Analyzing kills data for {args.player}...")
    kills_data = extract_kills_data(player_puuid, args.matches_dir)
    ...
    plot_kills_analysis(args.player, kills_data)
    
    if args.detailed:
        plot_detailed_performance(args.player, kills_data)
