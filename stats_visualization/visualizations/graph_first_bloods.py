...
def extract_early_game_data(player_puuid: str, matches_dir: str = "matches") -> Dict[str, Any]:
    """
...
def plot_first_blood_analysis(player_name: str, early_game_data: Dict[str, Any]):
    ...
    # First blood statistics
    fb_categories = ['First Blood Kills', 'First Blood Assists', 'First Blood Deaths', 'First Tower Kills']
    fb_counts = [
    ...
    ax2.hist(early_game_data['early_kills'], bins=range(0, max(early_game_data['early_kills']) + 2), 
    ...
    ax3.hist(early_game_data['early_deaths'], bins=range(0, max(early_game_data['early_deaths']) + 2), 
    ...
    for i, (fb_kill, fb_assist, fb_death, win) in enumerate(zip(
...
def plot_role_early_game_comparison(player_name: str, early_game_data: Dict[str, Any]):
    ...
    for role, kills, deaths, cs in zip(early_game_data['roles'], 
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate personal early game and first blood statistics visualization")
    ...
    parser.add_argument("--role-comparison", action="store_true",
                       help="Show role comparison")
    ...
    # Extract early game data
    print(f"Analyzing early game data for {args.player}...")
    ...
    plot_first_blood_analysis(args.player, early_game_data)
    
    if args.role_comparison:
        plot_role_early_game_comparison(args.player, early_game_data)
